...
class LogManager;
class CommitTracker;
// 统一存储访问门面（main 线引入）。TableHeap 内部仍直接持有 BufferPoolManager，
// 门面构造重载只是从 StorageAccess 中取出其包裹的 BPM，便于上层经门面建堆。
class StorageAccess;
...
public:
    ...
    // ---- StorageAccess 门面重载（main 线引入的统一存储访问门面）----
    // 语义与上面三个 BufferPoolManager* 版本完全一致，仅多一层门面解包：
    // storage 为 nullptr 时内部 BPM 亦为 nullptr（建堆后任何页访问都会失败）。
    ...
    //
    // out_new_rid：delete+insert 迁移路径下原 slot 被墓碑化、行被搬到新 slot，
    // RID 因此改变。需要正确 RID 的调用方（如 InsertIntoIndexes）必须使用这里
    // 输出的新 RID；传 nullptr 时行为等价于旧 API（不返回新 RID）。
