...
void UpsertExecutor::UpdateConflictingRow(const std::vector<Value>& candidate_values,
    ...
    // Phase A：事务已提前挂到堆上（读行时已记录快照读基）。UpdateTuple 这里直接抓
    // undo。同 UpdateExecutor：UpdateTuple 在行增长时走 delete+insert，RID 会变；
    ...
    heap->SetActiveTransaction(nullptr);
    if (ok) {
        // RID 未变（原位更新）时跳过键未变的重复插入；RID 已变时必须按新 RID
        // 写入，否则索引键指向墓碑槽。
        const std::vector<Value>* old_row_for_skip =
            (new_rid == existing_rid) ? &old_vals : nullptr;
        InsertIntoIndexes(context_->GetCatalog(), *info,
                          new_vals, new_rid, txn, old_row_for_skip);
        EmitReturning(new_t);
    } else {
