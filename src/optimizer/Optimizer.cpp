...
PlanNodePtr Optimizer::Optimize(PlanNodePtr plan) {
    if (!plan) return plan;
    // 优化顺序（U3 与 main 侧优化 pass 的并集，各 pass 的相对次序按语义依赖固定）：
    //   1) U3-3 子查询去关联 —— 必须在 Filter 谓词下推前处理，保证合取项仍是
    //      「Filter 直接持有的原始谓词」形态（下推会把谓词搬进 SeqScan.predicate）。
    //   2) 常量折叠 —— 先把恒真/恒假的 Filter 消掉，减少后续 pass 的无效遍历。
    //   3) U3-1 JOIN 重排 —— 全 INNER 左深链小表驱动。
    //   4) U3-2 聚合下推 —— 必须在 ChooseAccessPaths 之前：资格判定依赖改写前的
    //      「Filter*/SeqScan」单表形态，若先转成 IndexScan 则不再合格。
    //   5) 访问路径选择 —— Filter→IndexScan（复合索引最左前缀区间推导）。必须早于
    //      PushDownPredicates：后者会把 Filter 搬进 SeqScan.predicate，届时
    //      ChooseAccessPaths 只认「Filter 直接盖在 SeqScan 上」的形态，索引改写会全部失效。
    //   6) 常量折叠 + 谓词下推 + 常量折叠（下推后的 SeqScan.predicate 也可能有常量）。
    //   7) 列裁剪。
    plan = DecorrelateSubqueries(plan);
    plan = FoldConstants(plan);
    plan = ReorderJoins(plan);
    plan = PushDownAggregates(plan);
...
void Optimizer::PruneNode(PlanNode* node, const PruneColumnsCtx& ctx) {
    ...
    switch (type) {
        ...
        // U3-2 聚合下推后的扫描内预聚合节点（继承 AggregateNode，输出形状一致）：
        // 列需求与 AGGREGATE 完全相同——GROUP BY 列 + 聚合参数列都要由子 SeqScan 读出。
